...
export const SignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Authenticate and create user in FarmChain DB
      const res = await firebaseLogin(token, formData.fullName, formData.phone, formData.role);
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      // We pass the default selected role for Google signups
      const res = await firebaseLogin(token, formData.fullName, formData.phone, formData.role);
      if (res.success && res.data.accessToken) {
        localStorage.setItem('token', res.data.accessToken);
        navigate('/dashboard');
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center p-6 sm:p-12 lg:p-20 relative bg-cream">
        ...
        >
          ...
          <motion.div variants={itemVariants}>
            <h1 className="text-3xl md:text-4xl font-heading text-soil-900 mb-2 font-bold">Create Account</h1>
            <p className="text-soil-600 font-body mb-8 text-base">Join the modern agricultural network.</p>
          </motion.div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <motion.div variants={itemVariants} className="pt-2 space-y-3">
              ...
              >
                {loading ? <LeafLoader className="text-white" /> : 'Create Account with Email'}
              </motion.button>
